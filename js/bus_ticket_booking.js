...
function registerReservation(id){
     ...
     $.ajax({
          url:baseURL+"buildBusTicket.php",
          method:"GET",
...
async function buildTicket(trip_id){
    swal("Oops!, Sorry, but bus booking has been tempolarily disabled. Thank you for your support!!!");
    return;
     loading("show", "Processing Request...");
     $.get(baseURL+"buildBusTicket.php?step=one&trip_id="+trip_id, function(response){
          var json = JSON.parse(response);
          if(json.status === "success"){
